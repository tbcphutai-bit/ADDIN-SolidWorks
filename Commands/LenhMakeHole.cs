...
internal class LenhMakeHole
{
	...
	private bool TryCreateStraightEdgeOffsetAttempt(ModelDoc2 model, Face2 face, Edge edge, EdgeGeometry edgeGeometry, Feature seedFeature, MakeHoleOptions options, double signedDistance, bool requireStraightLine, out int pointCount, out string message)
	{
        ...
        SketchPoint seedPatternPoint = TryCreateSeedPatternPointInActiveSketch(model, list3);
        Debug.WriteLine("[MAKE HOLE] Line Flow seed point in 2D sketch=" + (seedPatternPoint != null));
	...
	private bool TryCreateNativeStraightEdgeOffsetInActiveSketch(ModelDoc2 model, Sketch activeSketch, Face2 face, Edge edge, EdgeGeometry edgeGeometry, double apiOffsetDistance, double desiredSignedDistance, bool requireStraightLine, out SketchSegment offsetSegment)
	{
		...
		try
		{
			...
			List<SketchSegment> sketchSegments = GetSketchSegments(activeSketch);
			offsetSegment = (requireStraightLine ? FindBestOffsetSegment(sketchSegments, activeSketch, edgeGeometry, Math.Abs(desiredSignedDistance), face) : FindLongestSketchSegment(sketchSegments));
			bool flag = offsetSegment != null && IsOffsetSegmentOnDesiredSide(activeSketch, offsetSegment, edgeGeometry, face, desiredSignedDistance, requireStraightLine ? "Line Flow native" : "Spline Flow native");
			Debug.WriteLine("[MAKE HOLE] Native " + (requireStraightLine ? "Line Flow" : "Spline Flow") + " offset. ok=" + flag + ", apiDistance=" + (apiOffsetDistance * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "mm, desiredSide=" + (desiredSignedDistance * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "mm, segments=" + sketchSegments.Count);
			if (!flag)
			{
	...
	private bool IsOffsetSegmentOnDesiredSide(Sketch activeSketch, SketchSegment offsetSegment, EdgeGeometry edgeGeometry, Face2 face, double desiredSignedDistance, string label)
	{
		...
		List<double[]> sourcePoints = SampleCurve(edgeGeometry, 96);
		List<double[]> offsetPoints = GetSketchSegmentModelPoints(activeSketch, offsetSegment);
		if (sourcePoints.Count < 2 || offsetPoints == null || offsetPoints.Count < 2)
		{
			Debug.WriteLine("[MAKE HOLE] Offset side check skipped. label=" + label + ", reason=not enough sample points");
			return true;
		}
		List<double> signedDistances = new List<double>();
		int step = Math.Max(1, offsetPoints.Count / 12);
		for (int i = 0; i < offsetPoints.Count; i += step)
		{
			...
			double[] sourcePoint = sourcePoints[Math.Max(0, Math.Min(sourcePoints.Count - 1, nearestPointIndex))];
		...
		{
			Debug.WriteLine("[MAKE HOLE] Offset side check skipped. label=" + label + ", reason=no signed samples");
			return true;
		}
		signedDistances.Sort((left, right) => Math.Abs(right).CompareTo(Math.Abs(left)));
		int count = Math.Max(1, signedDistances.Count / 2);
		double num = 0.0;
		for (int j = 0; j < count; j++)
		{
			num += signedDistances[j];
		}
		double representativeSignedDistance = num / count;
		double maxAbsSignedDistance = signedDistances[0];
		double minAcceptedDistance = Math.Max(0.001, Math.Abs(desiredSignedDistance) * 0.35);
		bool flag = Math.Abs(representativeSignedDistance) >= minAcceptedDistance && representativeSignedDistance * desiredSignedDistance > 0.0;
		Debug.WriteLine("[MAKE HOLE] Offset side check. label=" + label + ", ok=" + flag + ", desired=" + (desiredSignedDistance * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "mm, actualSide=" + (representativeSignedDistance * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "mm, maxSide=" + (maxAbsSignedDistance * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "mm, min=" + (minAcceptedDistance * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "mm, samples=" + signedDistances.Count + ", used=" + count);
		return flag;
	}
	...
	private bool TryCreateOffsetOnSurfaceAttempt(ModelDoc2 model, Face2 face, Edge edge, Feature seedFeature, EdgeGeometry edgeGeometry, MakeHoleOptions options, double offsetDistance, bool reverse, bool chain, out int pointCount)
	{
		...
		if (circleHole)
		{
			...
			AddPatternDirectionPointsToActiveSketch(model, list2);
		}
		model.SketchManager.Insert3DSketch(UpdateEditRebuild: true);
		if (circleHole)
		{
    ...
    private SketchPoint TryCreateSeedPatternPointInActiveSketch(ModelDoc2 model, List<double[]> holePoints)
    {
        ...
        try
        {
            SketchPoint sketchPoint = model.SketchManager.CreatePoint(point[0], point[1], point[2]);
            Debug.WriteLine("[MAKE HOLE] Seed pattern point created in active 2D sketch at (" + (point[0] * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "," + (point[1] * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + "," + (point[2] * 1000.0).ToString("0.###", CultureInfo.InvariantCulture) + ")");
            return sketchPoint;
        }
        catch (Exception ex)
        {
            Debug.WriteLine("[MAKE HOLE] Seed pattern point in active 2D sketch failed: " + ex.Message);
            return null;
        }
    }
	...
	private string TryCreateCurveLengthDimension(ModelDoc2 model, List<SketchSegment> segments)
	{
		...
		try
		{
			...
			TryHideReferenceLengthDimension(model, displayDimension);
 ...
 private string NormalizePaintHoleSizeText(string text)
	...
	private void TryEnableCurvePatternEqualSpacingOption()
	{
		try
		{
			...
			foreach (AutomationElement item in automationElementCollection)
			{
				...
				if (text.IndexOf("Equal", StringComparison.OrdinalIgnoreCase) < 0 && text2.IndexOf("EqualSpacing", StringComparison.OrdinalIgnoreCase) < 0)
				{
					continue;
				}
	...
	private void TryDisableCurvePatternGeometryOption()
	{
		try
		{
			...
			foreach (AutomationElement item in automationElementCollection)
			{
				...
				if (text.IndexOf("Geometry", StringComparison.OrdinalIgnoreCase) < 0 && text.IndexOf("Geometry Pattern", StringComparison.OrdinalIgnoreCase) < 0)
				{
					continue;
				}
				object patternObject;
	...
	private bool TryRepairHolesFromPlanarFace(ModelDoc2 model, Face2 face, double diameterM, double depthM, out int repairedCount, out string message)
	{
		...
		TryGroupRepairHoleReferenceFeatures(model, temporaryFillFeatures, repairPointFeatures, diameterM, circleCount);
